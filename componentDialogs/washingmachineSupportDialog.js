...
const washingCard = require('../resources/adaptiveCards/washingCard');
...
class WashingmachineSupportDialog extends ComponentDialog {
    
    constructor(conservsationState,userState) {
        super('washingmachineSupportDialog');
...
this.addDialog(new WaterfallDialog(WATERFALL_DIALOG, [
    ...
    this.getwashingmachineSupport.bind(this),
...
await step.context.sendActivity({
    text:'Samsung Washing Machine:',
    attachments: [CardFactory.adaptiveCard(CARDS[0])]
});

// Running a prompt here means the next WaterfallStep will be run when the users response is received.
return await step.prompt(CONFIRM_PROMPT, 'Would you like to Continue with Washing Machine Support?', ['yes', 'no']);
      
}

async getwashingmachineSupport(step){
...
return await step.prompt(CHOICE_PROMPT,'Please choose the Support From the listed below', ['NewInstallationDemo','ForRegisterNewRepair','ExtendedWarranty']);
    ...
    { 
        await step.context.sendActivity("You chose not to go ahead with Washing MAchine Support.");
        endDialog = true;
...
module.exports.WashingmachineSupportDialog = WashingmachineSupportDialog;
